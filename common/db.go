...
func NewDBJob(id *big.Int, streamID string,
	segmentCost *big.Int, profiles []ffmpeg.VideoProfile,
...
func InitDB(dbPath string) (*DB, error) {
	...
	stmt, err = db.Prepare("INSERT INTO jobs(id, streamID, segmentCost, transcodeOptions, broadcaster, transcoder, startBlock, endBlock) VALUES(?, ?, ?, ?, ?, ?, ?, ?)")
	if err != nil {
	...
	stmt, err = db.Prepare("SELECT id, streamID, segmentCost, transcodeOptions, broadcaster, transcoder, startBlock, endBlock FROM jobs WHERE endBlock > ? AND stopReason IS NULL")
	if err != nil {
...
func (db *DB) InsertJob(job *DBJob) error {
	...
	_, err := db.insertJob.Exec(job.ID, job.streamID, job.cost, options,
...
func (db *DB) ActiveJobs(since *big.Int) ([]*DBJob, error) {
	...
	for rows.Next() {
		...
		var options string
		if err := rows.Scan(&job.ID, &job.streamID, &job.cost, &options, &broadcaster, &transcoder, &job.startBlock, &job.endBlock); err != nil {
...
func (db *DB) ReceiptsByClaim(claimID int64, jobID *big.Int) ([]string, error) {
